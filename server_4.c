...
void *AtenderCliente (void *socket){
	...
	while(terminar == 0){
		...
		//aqui reenviamos el dibujo a todos los jugadores de la sala
			...
			char color[4098];
			char a[4098];
			int num_puntos_color;
			char points_drawing[4098];
			...
			p = strtok(NULL, "/");
			num_puntos_color= atoi(p);
			//int i = 0;
			
			strcpy(respuesta,"");
			sprintf(respuesta, "13/%s/%d/%s/%s/%d", nombre_jugador, num_sala, color, a, num_puntos_color);
			
			char secuencia_puntos[20];
			
			int i =0;
			
			while(i < num_puntos_color)
			{
				p = strtok(NULL, "/");
				sprintf(secuencia_puntos, "/%s", p);
				strcat(respuesta, secuencia_puntos);
				i++;
			}
